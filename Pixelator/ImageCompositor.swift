...
struct ImageCompositor {
    ...
    func flatten(sourceImage: CGImage, regions: [PixelatedRegion]) -> CGImage? {
        ...
        for region in regions {
            ...
            let drawRect = CGRect(
                ...
                y: CGFloat(height) - region.rect.origin.y - region.rect.height,
    ...
    private func applyPixellate(to image: CGImage, region: PixelatedRegion) -> CGImage? {
        ...
        let ciRect = CGRect(
            x: region.rect.origin.x,
            y: imageHeight - region.rect.origin.y - region.rect.height,
            width: region.rect.width,
            height: region.rect.height
        )

        let filter = CIFilter.pixellate()
        filter.inputImage = full.cropped(to: ciRect)
        filter.scale = Float(region.pixelSize)
        filter.center = CGPoint(
            x: region.rect.midX,
            y: imageHeight - region.rect.midY
        )
